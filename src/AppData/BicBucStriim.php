...
class BicBucStriim
{
    ...
    public function saveConfigs($configs)
    {
        foreach ($configs as $name => $val) {
            ...
            if (is_null($config)) {
                /** @var Config @config */
                $config = Config::cast(R::dispense('config'));
                $config->name = $name;
                $config->val = $val;
            } else {
                $config->val = $val;
    ...
    public function addUser($username, $password)
    {
        ...
        /** @var User $user */
        $user = User::cast(R::dispense('user'));
    ...
    public function addIdTemplate($name, $value, $label)
    {
        ...
        $template = Idtemplate::cast(R::dispense('idtemplate'));
        $template->name = $name;
        $template->val = $value;
    ...
    {
        /** @var Calibrething $calibreThing */
        $calibreThing = Calibrething::cast(R::dispense('calibrething'));
    ...
    public function editAuthorThumbnail($authorId, $authorName, $clipped, $file, $mime)
    {
        ...
        if (is_null($artefact)) {
            ...
            $artefact = Artefact::cast(R::dispense('artefact'));
    ...
    public function addAuthorLink($authorId, $authorName, $label, $url)
    {
        ...
        /** @var Link $link */
        $link = Link::cast(R::dispense('link'));
    ...
    public function editAuthorNote($authorId, $authorName, $mime, $noteText)
    {
        ...
        if (is_null($note)) {
            /** @var Note $note */
            $note = Note::cast(R::dispense('note'));
