...
interface RedBean_Driver
{
     ...
     */
    public function GetAll($sql, $aValues = []);
    /**
     ...
     */
    public function GetCell($sql, $aValues = []);
    /**
     ...
     */
    public function GetRow($sql, $aValues = []);
    /**
     ...
     * @return void
     */
    public function Execute($sql, $aValues = []);
    /**
    ...
    protected function runQuery($sql, $aValues)
    {
        ...
        try {
            if (strpos('pgsql', $this->dsn) === 0) {
                $s = $this->pdo->prepare($sql, [PDO::PGSQL_ATTR_DISABLE_NATIVE_PREPARED_STATEMENT => true]);
            } else {
...
class RedBean_OODBBean implements IteratorAggregate, ArrayAccess, Countable
{
     ...
     * @var array
     */
    private static $beautifulColumns = [];
    /**
     * @var boolean
     */
    ...
    */
     ...
     * @return RedBean_OODBBean $self
     */
    public function with($sql, $params = [])
    {
        if ($sql instanceof RedBean_SQLHelper) {
            [$this->withSql, $this->withParams] = $sql->getQuery();
        } else {
            $this->withSql = $sql;
     ...
     * @return RedBean_OODBBean $self
     */
    public function withCondition($sql, $params = [])
    {
        if ($sql instanceof RedBean_SQLHelper) {
    ...
    public function &__get($property)
    {
        ...
        if (!isset($this->properties[$property])) {
            ...
            if (isset($this->$fieldLink) && $fieldLink !== $this->getMeta('sys.idfield')) {
                ...
                if (!count($keys)) {
                    $beans = [];
                } elseif (trim($this->withSql) !== '') {
                    $beans = $redbean->find($type, ['id' => $keys], [$this->withSql, $this->withParams], true);
                } else {
...
interface RedBean_Adapter
{
     ...
     */
    public function exec($sql, $aValues = [], $noevent = false);
    /**
     ...
     * @param array  $aValues values
     */
    public function get($sql, $aValues = []);
    /**
     * Executes an SQL Query and returns a resultset.
...
class RedBean_Adapter_DBAdapter extends RedBean_Observable implements RedBean_Adapter
{
     ...
     */
    public function getAssoc($sql, $aValues = [])
    {
        $this->sql = $sql;
        $this->signal('sql_exec', $this);
     ...
     */
    public function getCell($sql, $aValues = [], $noSignal = null)
    {
        $this->sql = $sql;
        if (!$noSignal) {
            $this->signal('sql_exec', $this);
...
abstract class RedBean_QueryWriter_AQueryWriter
{
    ...
    public function updateRecord($type, $updatevalues, $id = null)
    {
        ...
        if (!$id) {
            ...
            return $this->insertRecord($table, $insertcolumns, [$insertvalues]);
        }
    ...
    public function addConstraint(RedBean_OODBBean $bean1, RedBean_OODBBean $bean2)
    {
        ...
        $table = RedBean_QueryWriter_AQueryWriter::getAssocTableFormat([$table1, $table2]);
...
class RedBean_QueryWriter_MySQL extends RedBean_QueryWriter_AQueryWriter implements RedBean_QueryWriter
{
    ...
    public const C_DATATYPE_UINT8 = 1;
    public const C_DATATYPE_UINT32 = 2;
    public const C_DATATYPE_DOUBLE = 3;
    public const C_DATATYPE_TEXT8 = 4;
    public const C_DATATYPE_TEXT16 = 5;
    public const C_DATATYPE_TEXT32 = 6;
    ...
    public function sqlStateIn($state, $list)
    {
        ...
        return in_array(($stateMap[$state] ?? '0'), $list);
    }
    /**
...
class RedBean_QueryWriter_SQLiteT extends RedBean_QueryWriter_AQueryWriter implements RedBean_QueryWriter
{
    ...
    protected function getTable($type)
    {
        ...
        $table = ['columns' => $columns, 'indexes' => $indexes, 'keys' => $keys, 'name' => $tableName];
...
class RedBean_OODB extends RedBean_Observable
{
     ...
     */
    public function find($type, $conditions = [], $addSQL = null, $all = false)
    {
        try {
    ...
    public function store($bean)
    {
        ...
        if ($processLists) {
            //Define groups
            $sharedAdditions = $sharedTrashcan = $sharedresidue = $sharedItems = [];
            ...
            foreach ($bean as $p => $v) {
                ...
                if (is_array($v)) {
                    ...
                    if (strpos($p, 'own') === 0) {
                        [$ownAdditions, $ownTrashcan, $ownresidue] = $this->processGroups($originals, $v, $ownAdditions, $ownTrashcan, $ownresidue);
                        $bean->removeProperty($p);
                    } elseif (strpos($p, 'shared') === 0) {
                        [$sharedAdditions, $sharedTrashcan, $sharedresidue] = $this->processGroups($originals, $v, $sharedAdditions, $sharedTrashcan, $sharedresidue);
                        $bean->removeProperty($p);
                    } else {
                    }
                }
        ...
        if ($bean->getMeta('tainted')) {
            ...
            foreach ($bean as $p => $v) {
                ...
                if ($p !== 'id') {
                    ...
                    $insertvalues[] = $v;
                    $insertcolumns[] = $p;
                    $updatevalues[] = ['property' => $p, 'value' => $v];
                }
            }
    ...
    {
        return (bool) (isset($this->dep[$refType]) && in_array($otherType, $this->dep[$refType]));
    }
    /**
    ...
    public function load($type, $id)
    {
        ...
        if (isset($this->stash[$this->nesting][$id])) {
            ...
            try {
                $rows = $this->writer->selectRecord($type, ['id' => [$id]]);
            } catch(RedBean_Exception_SQL $e) {
                if (
    ...
    public function batch($type, $ids)
    {
        ...
        $collection = [];
        try {
            $rows = $this->writer->selectRecord($type, ['id' => $ids]);
        } catch(RedBean_Exception_SQL $e) {
            if (!$this->writer->sqlStateIn(
                $e->getSQLState(),
                [
     ...
     */
    public function count($type, $addSQL = '', $params = [])
    {
        try {
            return (int) $this->writer->count($type, $addSQL, $params);
        } catch(RedBean_Exception_SQL $e) {
            if (!$this->writer->sqlStateIn($e->getSQLState(), [RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_TABLE])) {
                throw $e;
            }
        }
        return 0;
    ...
    public function preload($beans, $types, $closure = null)
    {
        ...
        foreach ($types as $key => $typeInfo) {
            [$type, $sqlObj] = (is_array($typeInfo) ? $typeInfo : [$typeInfo, null]);
            ...
            foreach ($filteredBeans as $bean) {
                $bean->setMeta('sys.input-bean-id', [$counterID => $counterID]);
                $counterID++;
            }
            ...
            if (strpos($field, 'shared') === 0) {
                ...
                $sharedBeans = $this->find($type, ['id' => $targetIDs], $sqlObj, (!is_null($sqlObj)));
...
class RedBean_AssociationManager extends RedBean_Observable
{
    ...
    public function related($bean, $type, $getLinks = false, $sql = false)
    {
        ...
        try {
            ...
            if ($cross) {
                $sqlFetchKeys2 = $this->writer->selectRecord($table, [$targetproperty => $ids], $sql, false);
        ...
        foreach ($beans1 as $bean1) {
            foreach ($beans2 as $bean2) {
                ...
                try {
                    ...
                    if ($cross) {
                        $rows2 = $this->writer->selectRecord(
                            $table,
                            [
                        $property2 => [$value1], $property1 => [$value2]],
                            null,
                            $fast
                        );
                        if ($fast) {
    ...
    public function clearRelations(RedBean_OODBBean $bean, $type)
    {
        ...
        try {
            ...
            if ($cross) {
                $this->writer->selectRecord($table, [$property2 => [$bean->id]], null, true);
            }
        } catch(RedBean_Exception_SQL $e) {
            if (!$this->writer->sqlStateIn(
                $e->getSQLState(),
     ...
     */
    public function relatedSimple($bean, $type, $sql = null, $values = [])
    {
        ...
        $rows = $this->writer->selectRecord($type, ['id' => $keys], [$sql, $values], false);
    ...
    */
    public function relatedOne(RedBean_OODBBean $bean, $type, $sql = null, $values = [])
    {
     ...
     */
    public function unrelated(RedBean_OODBBean $bean, $type, $sql = null, $values = [])
    {
        $keys = $this->related($bean, $type);
        $rows = $this->writer->selectRecord($type, ['id' => $keys], [$sql, $values], false, true);
        return $this->oodb->convertToBeans($type, $rows);
    }
}
...
class RedBean_Finder
{
     ...
     * @return array $beans  beans
     */
    public function find($type, $sql = null, $values = [])
    {
        if ($sql instanceof RedBean_SQLHelper) {
        ...
        return $this->redbean->find($type, [], [$sql, $values]);
    }
    /**
     ...
     * @return array $beans  beans
     */
    public function findAll($type, $sql = null, $values = [])
    {
        if ($sql instanceof RedBean_SQLHelper) {
            [$sql, $values] = $sql->getQuery();
        }
     ...
     */
    public function findAndExport($type, $sql = null, $values = [])
    {
     ...
     * @return RedBean_OODBBean $bean
     */
    public function findOne($type, $sql = null, $values = [])
    {
        $items = $this->find($type, $sql, $values);
        $found = reset($items);
        if (!$found) {
     ...
     * @return RedBean_OODBBean $bean
     */
    public function findLast($type, $sql = null, $values = [])
    {
        $items = $this->find($type, $sql, $values);
        $found = end($items);
        if (!$found) {
     ...
     */
    public function findOrDispense($type, $sql = null, $values = [])
    {
...
class RedBean_Facade
{
    ...
    */
     ...
     * @return array $beans Contains RedBean_OODBBean instances
     */
    public static function findOrDispense($type, $sql = null, $values = [])
    {
     ...
     * @return array $beans	beans yielded by your query.
     */
    public static function related($bean, $type, $sql = null, $values = [])
    {
    ...
    * @return RedBean_OODBBean $bean
    */
    public static function relatedOne(RedBean_OODBBean $bean, $type, $sql = null, $values = [])
    {
     ...
     * @return array $beans beans
     */
    public static function unrelated(RedBean_OODBBean $bean, $type, $sql = null, $values = [])
    {
     ...
     * @return array $beans  beans
     */
    public static function find($type, $sql = null, $values = [])
    {
     ...
     * @return array $beans  beans
     */
    public static function findAll($type, $sql = null, $values = [])
    {
     ...
     * @return array $arrays arrays
     */
    public static function findAndExport($type, $sql = null, $values = [])
    {
     ...
     * @return RedBean_OODBBean $bean
     */
    public static function findOne($type, $sql = null, $values = [])
    {
     ...
     * @return RedBean_OODBBean $bean
     */
    public static function findLast($type, $sql = null, $values = [])
    {
     ...
     */
    public static function getAll($sql, $values = [])
    {
     ...
     */
    public static function getCell($sql, $values = [])
    {
     ...
     * @return array $results
     */
    public static function getRow($sql, $values = [])
    {
     ...
     * @return array $results
     */
    public static function getCol($sql, $values = [])
    {
     ...
     */
    public static function count($beanType, $addSQL = '', $params = [])
    {
        ...
        try {
            switch($action) {
                ...
                case 'export':
                    ...
                    $bean = RedBean_Facade::load($beanType, $data[0]);
                    $array = RedBean_Facade::exportAll([$bean], true);
...
class RedBean_DuplicationManager
{
    ...
    {
        return (in_array(RedBean_QueryWriter_AQueryWriter::getAssocTableFormat([$type, $target]), $this->tables));
    }
    /**
     * Makes a copy of a bean. This method makes a deep copy
     * of the bean.The copy will have the following features.
     ...
     * @return array $copiedBean the duplicated bean
     */
    public function dup($bean, $trail = [], $pid = false)
    {
